...
graph(%a.1 : Tensor,
      %b.1 : Tensor):
  %t0 : Float(2:2, 2:1, device=cuda:0, requires_grad=1), %t1 : Float(3:3, 3:1), %type_matched : bool = prim::TypeCheck(%a.1, %b.1)
  return (%t0, %t1, %type_matched)
    ...
    }
    {
      ...
      // size (3,3) got (2,2). Tensor b is returned
      auto a = at::zeros({2, 2});
    ...
    }
    {
      ...
      auto b = at::ones({3, 3});
      a.set_requires_grad(true);
      a = a.to(at::kCPU); // device mismatch
      std::vector<IValue> stack({a, b});
      interp.run(stack);
      ASSERT_FALSE(stack[2].toBool());
    }
